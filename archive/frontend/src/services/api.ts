...
api.interceptors.response.use(
  ...
  (error: unknown) => {
    if (error instanceof AxiosError) {
      ...
      // Log errors in development - removed console logging
    }
    return Promise.reject(error);
  }
);
...
export const groupsAPI = {
  ...
  // Get group info for invite preview (public groups only, uses direct axios since auth not required)
...
export const sessionsAPI = {
  ...
  joinViaInvite: (id: string | number) => api.post(`/sessions/${id}/join-invite`),
  leave: (id: string | number) => api.delete(`/sessions/${id}/leave`),
  deleteAttendance: (sessionId: string | number, userId: string | number) => api.delete(`/sessions/${sessionId}/attendance/${userId}`),
