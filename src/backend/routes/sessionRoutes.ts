...
import * as eventController from '../controllers/sessionController';
...
router.post('/:id/invite', noCache, asyncHandler(eventController.inviteToEvent));
router.post('/:id/invitations/revoke', noCache, asyncHandler(eventController.revokeEventInvitation));
router.get('/:id/invitations/analytics', etagMiddleware({ weak: true }), cacheControl(120, { private: true }), asyncHandler(eventController.getEventInviteAnalytics));
router.post('/:id/invitations/generate-token', noCache, asyncHandler(eventController.generateEventInviteToken));
router.put('/:id/status', noCache, asyncHandler(eventController.updateParticipationStatus));
router.put('/:id/session-status', noCache, asyncHandler(eventController.updateSessionStatus));
...
router.post('/:sessionId/reminders', noCache, reminderController.createReminder);
router.get('/:sessionId/reminders', cacheControl(120, { private: true }), reminderController.getEventReminders);

// Event attendance routes
router.post('/:sessionId/attendance', noCache, attendanceController.markAttendance);
router.get('/:sessionId/attendance', cacheControl(60, { private: true }), attendanceController.getEventAttendance);
router.get('/:sessionId/attendance/stats', cacheControl(180, { private: true }), attendanceController.getAttendanceStats);
router.delete('/:sessionId/attendance/:userId', noCache, attendanceController.deleteAttendance);
